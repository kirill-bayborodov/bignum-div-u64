/**
 * @file test_bignum_div_u64_benchmark_adapter.c
 * @brief Deterministic tests for the bignum_div_u64 benchmark adapter.
...
static void test_validation(void)
{
    ...
    assert(bignum_div_u64_benchmark_validate_workload(&workload) ==
        BIGNUM_DIV_U64_BENCHMARK_STATUS_SUCCESS);
    invalid.operation_kind = "xor";
    assert(bignum_div_u64_benchmark_validate_workload(&invalid) ==
        BIGNUM_DIV_U64_BENCHMARK_STATUS_INVALID_PROFILE);
    assert(bignum_div_u64_benchmark_validate_workload(NULL) ==
        BIGNUM_DIV_U64_BENCHMARK_STATUS_NULL_ARGUMENT);
}
...
static void test_callbacks(void)
{
    ...
    assert(bignum_div_u64_benchmark_adapter_init(NULL) ==
        BIGNUM_DIV_U64_BENCHMARK_STATUS_NULL_ARGUMENT);
    assert(bignum_div_u64_benchmark_adapter_init(&adapter) ==
        BIGNUM_DIV_U64_BENCHMARK_STATUS_SUCCESS);
...
int main(void)
{
    ...
    test_legacy_modes();
    test_callbacks();
    puts("bignum_div_u64 benchmark adapter tests: OK");
    return EXIT_SUCCESS;
}

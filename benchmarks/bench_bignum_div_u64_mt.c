...
#include "adapter/bignum_div_u64_benchmark_adapter.h"

int main(int argc, char **argv)
{
    benchmark_adapter_t adapter;
    bignum_div_u64_benchmark_status_t adapter_status;
    benchmark_core_status_t core_status;

    adapter_status = bignum_div_u64_benchmark_adapter_init(&adapter);
    if (adapter_status != BIGNUM_DIV_U64_BENCHMARK_STATUS_SUCCESS) return EXIT_FAILURE;
